...
function Frog:update(dt)
    ...
    Frog.timer = Frog.timer +((dt *1000)/60)
    if Frog.timer >= Frog.time *((dt *1000)/60) then
        Frog.timer = 0
        local x = 640 + 20
        ...
        bullet = require (mod_loaded .."scripts/battle/bullets/".. 'basicbullet')(x, y, math.rad(180), -8)
        table.insert(Frog.monster.bullets, bullet)
